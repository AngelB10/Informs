...
const AttendancePieChart = ({ type, mount }) => {
  ...
  useEffect(() => {
    ...
    const groupedData = [1, 2, 3, 4].map((week) => {
      const informesSemana = informes.filter(
        (informe) => informe.week == week && new Date(informe.date).getMonth() == mount
      );

      informesSemana.forEach((inf) => {
        const leaderInfo = leaders.find(
          (l) => l._id.trim().toLowerCase() === inf.leader.trim().toLowerCase()
        );
        if (leaderInfo) {
    ...
    const validWeeks = groupedData.filter(
      (weekData) =>
        weekData.asistencia.some((d) => d.value > 0) ||
        weekData.nuevos.some((d) => d.value > 0)
    );

    const lastWeekData =
      validWeeks.length > 0 ? validWeeks[validWeeks.length - 1] : { asistencia: [], nuevos: [] };

    setAttendanceData(lastWeekData[type]);
  }, [informes, leaders, type, mount]);
  ...
  return (
    <div className="bg-white p-4 shadow-md rounded-md">
      ...
      <h6>Información del mes: {mount + 1}</h6>
